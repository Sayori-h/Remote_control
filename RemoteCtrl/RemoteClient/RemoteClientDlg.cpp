...
void CRemoteClientDlg::UpdateFileInfo(const FILEINFO& finfo,HTREEITEM hParent)
{
	...
	if (finfo.isDirectory) {
		...
		HTREEITEM hTemp = m_Tree.InsertItem(finfo.szFileName, (HTREEITEM)hParent, TVI_LAST);
		//如果是目录，就有子节点;区分目录和文件，目录后面插NULL，文件不插
		m_Tree.InsertItem(NULL, hTemp, TVI_LAST);
		m_Tree.Expand(hParent, TVE_EXPAND);
	}
	else m_List.InsertItem(0, finfo.szFileName);//文件不在树上显示，树上只显示目录
}
...
{
	if (lParam == -1 || (lParam == -2))TRACE("socket is error %d\r\n", lParam);
	else if (lParam == 1)TRACE("socket is closed!\r\n");
	else{
		if (wParam) {
